...
namespace GPBoost {
	...
	*/
		...
		double REModel::TestNegLogLikelihoodGammaGamma(const label_t* y_test,
			const double* fixed_effects_test,
			const data_size_t num_data) {
			if (matrix_format_ == "sp_mat_t") {
				return(re_model_sp_->TestNegLogLikelihoodGammaGamma(y_test, fixed_effects_test, num_data, cov_pars_));
			}
			else if (matrix_format_ == "sp_mat_rm_t") {
				return(re_model_sp_rm_->TestNegLogLikelihoodGammaGamma(y_test, fixed_effects_test, num_data, cov_pars_));
			}
			else {
				return(re_model_den_->TestNegLogLikelihoodGammaGamma(y_test, fixed_effects_test, num_data, cov_pars_));
			}
		}
