				...
				if (weights_ == nullptr) {
					if (objective->HasGPModel() && objective->UseGPModelForValidation()) {
						...
						REModel* re_model = objective->GetGPModel();
							if (re_model->GaussLikelihood() && re_model->GetLikelihood() != std::string("gamma_gamma")) {//Gaussian data
	...
	public:
		...
		std::vector<double> Eval(const double* score, 
			...
			if (objective->HasGPModel()) {
				...
				double sum_loss = 0.;
				if (objective->HasGPModel() && objective->UseGPModelForValidation()) {
					if (re_model->GetLikelihood() == std::string("gamma_gamma")) {
						sum_loss = re_model->TestNegLogLikelihoodGammaGamma(label_, score, num_data_);
					}
					else if (re_model->GaussLikelihood()) {//Gaussian data
