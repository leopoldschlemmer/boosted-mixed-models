		...
		*/
		REModelTemplate(data_size_t num_data,
			...
			if (likelihood == nullptr) {
				gamma_gamma_model_ = false;
				likelihood_strg = "gaussian";
			}
			else {
				string_t likelihood_parsed = Likelihood<T_mat, T_chol>::ParseLikelihoodAlias(std::string(likelihood));
				gamma_gamma_model_ = (likelihood_parsed == "gamma_gamma");
				likelihood_strg = gamma_gamma_model_ ? "gaussian" : likelihood_parsed;
			}
			...
			gamma_gamma_alpha_ = likelihood_additional_param;
...
//				}
		...
		*/
		void SetLikelihood(const string_t& likelihood) {
			...
			string_t likelihood_parsed = Likelihood<T_mat, T_chol>::ParseLikelihoodAlias(likelihood);
			gamma_gamma_model_ = (likelihood_parsed == "gamma_gamma");
			string_t likelihood_strg = gamma_gamma_model_ ? "gaussian" : likelihood_parsed;
			gauss_likelihood_ = (likelihood_strg == "gaussian") && !vecchia_latent_approx_gaussian_;
		...
		*/
		...
		/*!
		* \brief Find covariance parameters and linear regression coefficients (if there are any) that minimize the (approximate) negative log-likelihood
		...
		*/
		void OptimLinRegrCoefCovPar(const double* y_data,
			...
			// As otherwise the cov_pars[0] is treated like Gaussian error variance, when profile_out_error_variance_=True,
			// and for the custom Gamma-Gamma path, cov_pars[0] is the model parameter delta, we set profile_out_error_variance_=False
			// we also set the optimizer of the regression coefficients to "gradient_descent", 
			// as within the Gamma-Gamma setting, we provide the gradient for beta and we want to avoid Gaussian closed-form/WLS coefficient updates
			if (gamma_gamma_model_) {
				profile_out_error_variance_ = false;
			...
			if (y_data != nullptr) {
				...
				//Leopold:
				// Ensure y_ is initialized for any legacy helper that still accesses it.
				if (gamma_gamma_model_) {
					SetY(y_data);
				}
			}
...
#pragma omp critical
			...
			if (!has_covariates_ || !gauss_likelihood_) {
				...
				if (gauss_likelihood_ && !gamma_gamma_model_ && !find_learning_rate_for_GPBoost_algo) {
...
#pragma omp parallel for schedule(static)
			...
			if (estimate_aux_pars_) {
				...
				if (!AuxParsHaveBeenSetOrEstimated()) {//If initial values have been provided, these were set in re_model previously
					if (gamma_gamma_model_) {
						double alpha_init = GetGammaGammaAlpha();
						SetAuxPars(&alpha_init);
					}
					else {
		...
		*/
		...
		//Leopold:
		// In this model, cov_pars[0] is delta and likelihood_additional_param_ is alpha.
		void CalcGradParsGammaGamma(const vec_t& cov_pars,
									...
									const double* fixed_effects) {
			if (!gamma_gamma_model_) {
				Log::REFatal("Internal error: CalcGradParsGammaGamma called while Gamma-Gamma mode is disabled");
			}

			
			if (num_cov_par_ <= 0) {
				Log::REFatal("Gamma-Gamma: expected num_cov_par_ >= %d but got %d", 1, (int)num_cov_par_);
			}
			const double delta = GetGammaGammaDelta(cov_pars);
			const double alpha = GetGammaGammaAlpha();
			const double* eta_ptr = fixed_effects;
			vec_t eta_internal;
			if (!(alpha > 0.0)) {
				Log::REFatal("Gamma-Gamma: alpha (likelihood_additional_param_) must be > 0");
			}
			if (eta_ptr == nullptr) {
				if (!has_covariates_) {
					Log::REFatal("Gamma-Gamma: need covariates to build eta internally when fixed_effects is null");
				}
				if ((int)beta_.size() != num_covariates_) {
					Log::REFatal("Gamma-Gamma: beta_ size (%d) != num_covariates_ (%d)",
								(int)beta_.size(), (int)num_covariates_);
				}
				eta_internal = X_ * beta_;
				eta_ptr = eta_internal.data();
			}

			//Here we make sure that none of the more advanced features of GPBoost are being used, as we do not need them for our model.
			if (num_clusters_ != 1) {
				Log::REFatal("Gamma-Gamma: currently supports only one cluster");
			}
			if (num_re_group_total_ != 1 || num_gp_total_ != 0) {
				Log::REFatal("Gamma-Gamma: currently supports exactly one grouped RE and no GP");
			}

			const auto& cluster_i = unique_clusters_[0];
			auto re_comp = re_comps_[cluster_i][0][0];
			const int G = re_comp->GetNumUniqueREs();
			const auto& grp = re_comp->random_effects_indices_of_data_;

			if (calc_cov_aux_par_grad) {
				const int num_cov_aux_par = num_cov_par_ + (estimate_aux_pars_ ? NumAuxPars() : 0);
				if ((int)grad_cov_aux_par.size() != num_cov_aux_par) {
					grad_cov_aux_par = vec_t::Zero(num_cov_aux_par);
				} else {
					grad_cov_aux_par.setZero();
				}
			}
			if (calc_beta_grad) {
				grad_beta = vec_t::Zero(num_sets_re_ * num_covariates_);
			}
			...
			if (y_has_been_set_) {
				...
				Log::REFatal("Gamma-Gamma: response y must be set before CalcGradParsGammaGamma");
			}

			vec_t Sg = vec_t::Zero(G); // S_g = sum_j exp(F_gj) * y_gj
			vec_t SumF_g = vec_t::Zero(G);
			vec_t SumLogY_g = vec_t::Zero(G);
			vec_t Ng = vec_t::Zero(G); // n_g

			// Here we calculate the group statistics needed for the Gamma-Gamma gradients.
			#pragma omp parallel
			{
				vec_t Sg_loc = vec_t::Zero(G);
				vec_t SumF_loc = vec_t::Zero(G);
				vec_t SumLogY_loc = vec_t::Zero(G);
				vec_t Ng_loc = vec_t::Zero(G);

				#pragma omp for schedule(static)
				for (data_size_t i = 0; i < num_data_; ++i) {
					const double yi = y_ptr[i];
					if (!(yi > 0.0)) {
						Log::REFatal("Gamma-Gamma: response must be > 0");
					}
				...
				#pragma omp critical
				{
					Sg += Sg_loc;
					SumF_g += SumF_loc;
					SumLogY_g += SumLogY_loc;
					Ng += Ng_loc;
				}
			}

			// We calculate d(negll)/d(delta) and transform to d(negll)/d(log(delta)).
			if (calc_cov_aux_par_grad) {
				double grad_delta = 0.0;
				double grad_alpha = 0.0;

				#pragma omp parallel for schedule(static) reduction(+:grad_delta, grad_alpha)
				for (int g = 0; g < G; ++g) {
					const double S = Sg[g];
					const double n = Ng[g];
					const double n_alpha_plus_delta = n * alpha + delta;
					const double delta_plus_S = delta + S;
					grad_delta += -(std::log(delta) + 1.0)
								+ GPBoost::digamma(delta)
								- GPBoost::digamma(n_alpha_plus_delta)
								+ std::log(delta_plus_S)
								+ n_alpha_plus_delta / delta_plus_S;
					if (estimate_aux_pars_) {
						grad_alpha += n * GPBoost::digamma(alpha)
									- n * GPBoost::digamma(n_alpha_plus_delta)
									- SumF_g[g]
									- SumLogY_g[g]
									+ n * std::log(delta_plus_S);
					}
				}

				grad_cov_aux_par[0] = grad_delta * delta;
				if (estimate_aux_pars_) {
					grad_cov_aux_par[num_cov_par_] = grad_alpha * alpha;
				}
			}

			// Here we calculate the gradient w.r.t. the regression coefficients beta.
			if (calc_beta_grad) {
				if (num_sets_re_ != 1) {
					Log::REFatal("Gamma-Gamma: currently supports only num_sets_re_ == 1 for beta gradient");
				}

				vec_t grad_eta = vec_t::Zero(num_data_);
				#pragma omp parallel for schedule(static)
				for (data_size_t i = 0; i < num_data_; ++i) {
					const double yi = y_ptr[i];
					const double eta = eta_ptr[i];
					...
					const double n_alpha_plus_delta = Ng[g] * alpha + delta;
					const double delta_plus_S = delta + Sg[g];
					grad_eta[i] = -alpha + n_alpha_plus_delta * exp_eta * yi / delta_plus_S;
				}

				grad_beta.segment(0, num_covariates_) = (X_.transpose()) * grad_eta;
			}
		}
...
#pragma omp parallel for schedule(static)
		...
		*/
		bool AuxParsHaveBeenSetOrEstimated() {
			if (gamma_gamma_model_) {
				return gamma_gamma_aux_pars_have_been_set_;
			}
		...
		*/
		void SetAuxPars(const double* aux_pars) {
			if (gamma_gamma_model_) {
				CHECK(aux_pars != nullptr);
				// Be robust against optimizer proposals that underflow to 0 or produce invalid values.
				if (!std::isfinite(aux_pars[0]) || !(aux_pars[0] > 0.)) {
					gamma_gamma_alpha_ = 1e-12;
				}
				else {
					gamma_gamma_alpha_ = aux_pars[0];
				}
				gamma_gamma_aux_pars_have_been_set_ = true;
				return;
			}
			for (const auto& cluster_i : unique_clusters_) {
		...
		const char* GetNameAuxPars(int ind_aux_par) {
			if (gamma_gamma_model_) {
				CHECK(ind_aux_par == 0);
				return "alpha";
			}
			return likelihood_[unique_clusters_[0]]->GetNameAuxPars(ind_aux_par);
		}
		...
		*/
		void CalcCovFactorOrModeAndNegLL(const vec_t& cov_pars_in,
			...
			//Leopold:
			if (gamma_gamma_model_) {
				EvalNegLogLikelihoodGammaGamma(nullptr, neg_log_likelihood_, fixed_effects, cov_pars);
				return;
			}
		...
		*/
		void UpdateFixedEffects(const vec_t& beta,
			...
			vec_t& fixed_effects_vec) {
			//Leopold:
			// In our model we only need the num_sets_re=1 case, but I keep the code general just in case.
			if (gamma_gamma_model_) {
...
#pragma omp parallel for schedule(static)
		...
		//Leopold:
		// Note that in this model, cov_pars[0] is delta and likelihood_additional_param_ is alpha.
			void EvalNegLogLikelihoodGammaGamma(
				const double* y_data,
				double& negll,
				const double* fixed_effects,
				const vec_t& cov_pars_gg) {
			if (!gamma_gamma_model_) {
				Log::REFatal("Internal error: EvalNegLogLikelihoodGammaGamma called while Gamma-Gamma mode is disabled");
			}
			
			CHECK(num_sets_re_ == 1);
			CHECK((int)cov_pars_gg.size() >= 1);
			if (num_cov_par_ <= 0) {
				Log::REFatal("Gamma-Gamma: expected num_cov_par_ >= %d but got %d", 1, (int)num_cov_par_);
			}
			const double delta = GetGammaGammaDelta(cov_pars_gg);
			const double alpha = GetGammaGammaAlpha();
			if (!(alpha > 0.0)) {
				Log::REFatal("Gamma-Gamma: alpha (likelihood_additional_param_) must be > 0");
			}
			...
			if (eta_ptr == nullptr) {
				if (has_covariates_) {
					if ((int)beta_.size() != num_covariates_) {
						Log::REFatal("Gamma-Gamma: beta_ size (%d) != num_covariates_ (%d)",
								(int)beta_.size(), num_covariates_);
					}
					if (X_.rows() != num_data_ || X_.cols() != num_covariates_) {
						Log::REFatal("Gamma-Gamma: X_ has wrong dimension (%d,%d), expected (%d,%d)",
								(int)X_.rows(), (int)X_.cols(), (int)num_data_, (int)num_covariates_);
					}
					eta_internal = X_ * beta_;
					eta_ptr = eta_internal.data();
				} else {
					Log::REFatal("Gamma-Gamma: no fixed effects available");
				}
			}

			// get y pointer (either provided or already set)
			const double* y_ptr = y_data;
			if (y_ptr == nullptr) {
				if (!y_has_been_set_) {
				Log::REFatal("Gamma-Gamma: y_data is null and y has not been set before");
				}
				y_ptr = y_vec_.data();
			}
			...
			vec_t Sg = vec_t::Zero(G);      // S_g = sum_j exp(F_gj) * y_gj
			vec_t SumF_g = vec_t::Zero(G);  // sum_j F_gj
			vec_t SumLogY_g = vec_t::Zero(G); // sum_j log(y_gj)
			vec_t Ng = vec_t::Zero(G);      // n_g
			...
			double negll_sum = 0.0;
			#pragma omp parallel for schedule(static) reduction(+:negll_sum)
			for (int g = 0; g < G; ++g) {
				const double S = Sg[g];
				const double n = Ng[g];
				const double n_alpha_plus_delta = n * alpha + delta;
				negll_sum += n * std::lgamma(alpha)
							- delta * std::log(delta)
							+ std::lgamma(delta)
							- std::lgamma(n_alpha_plus_delta)
							- alpha * SumF_g[g]
							- (alpha - 1.0) * SumLogY_g[g]
							+ n_alpha_plus_delta * std::log(delta + S);
			}

				negll = negll_sum;
			}

			double TestNegLogLikelihoodGammaGamma(
				const label_t* y_test,
				const double* fixed_effects_test,
				data_size_t num_data_test,
				const vec_t& cov_pars_gg) {
				if (!gamma_gamma_model_) {
					Log::REFatal("Internal error: TestNegLogLikelihoodGammaGamma called while Gamma-Gamma mode is disabled");
				}
				if (y_test == nullptr || fixed_effects_test == nullptr) {
					Log::REFatal("Gamma-Gamma: test labels and fixed effects are required for test NLL");
				}
				if ((data_size_t)re_group_levels_pred_.size() < 1 || (data_size_t)re_group_levels_pred_[0].size() != num_data_test) {
					Log::REFatal("Gamma-Gamma: prediction group data must be set before calculating test NLL");
				}
				CHECK(num_sets_re_ == 1);
				CHECK((int)cov_pars_gg.size() >= 1);
				if (num_cov_par_ <= 0) {
				...
				if (!(alpha > 0.0) || !(delta > 0.0)) {
					Log::REFatal("Gamma-Gamma: alpha and delta must be > 0");
				}

				std::map<re_group_t, int> group_index;
				std::vector<int> group_id(num_data_test);
				int num_groups = 0;
				for (data_size_t i = 0; i < num_data_test; ++i) {
					const re_group_t& group_label = re_group_levels_pred_[0][i];
					auto it = group_index.find(group_label);
					if (it == group_index.end()) {
						group_index.insert({ group_label, num_groups });
				...
				vec_t Sg = vec_t::Zero(num_groups);
				vec_t SumF_g = vec_t::Zero(num_groups);
				vec_t SumLogY_g = vec_t::Zero(num_groups);
				vec_t Ng = vec_t::Zero(num_groups);

#pragma omp parallel
				{
					vec_t Sg_loc = vec_t::Zero(num_groups);
					vec_t SumF_loc = vec_t::Zero(num_groups);
					vec_t SumLogY_loc = vec_t::Zero(num_groups);
					vec_t Ng_loc = vec_t::Zero(num_groups);

#pragma omp for schedule(static)
					for (data_size_t i = 0; i < num_data_test; ++i) {
						const double yi = static_cast<double>(y_test[i]);
						if (!(yi > 0.0)) {
							Log::REFatal("Gamma-Gamma: response must be > 0");
						}
						const double eta = fixed_effects_test[i];
						const int g = group_id[i];
						Sg_loc[g] += std::exp(eta) * yi;
						SumF_loc[g] += eta;
						SumLogY_loc[g] += std::log(yi);
						Ng_loc[g] += 1.0;
					}
...
#pragma omp parallel for schedule(static) reduction(+:negll_sum)
				for (int g = 0; g < num_groups; ++g) {
					...
					negll_sum += n * std::lgamma(alpha)
						...
						- alpha * SumF_g[g]
						- (alpha - 1.0) * SumLogY_g[g]
						+ n_alpha_plus_delta * std::log(delta + Sg[g]);
				}
				return negll_sum;
			}
...
#pragma omp parallel for schedule(static)   
		...
		*/
		void EvalNegLogLikelihood(const double* y_data,
			...
			if (gamma_gamma_model_) { 
				CHECK(cov_pars != nullptr);
				const vec_t cov_pars_vec = Eigen::Map<const vec_t>(cov_pars, num_cov_par_);
				EvalNegLogLikelihoodGammaGamma(y_data, negll, fixed_effects, cov_pars_vec);
				return;
			}
		...
		*/
		void PrintTraceParameters(const vec_t& cov_pars_in,
			...
			if (Log::GetLevelRE() == LogLevelRE::Debug) { // do transformation only if log level Debug is active
				...
				if (estimate_aux_pars_ && print_cov_aux_pars) {
					...
					for (int i = 0; i < NumAuxPars(); ++i) {
						Log::REDebug("%s: %g", GetNameAuxPars(i), aux_pars_print_orig[i]);
					}
				}
			}
		...
		//Leopold:
		// In this model, cov_pars[0] is delta and likelihood_additional_param_ is alpha.
		void CalcGradientFGammaGamma(double* y,
									const double* fixed_effects,
									const vec_t& cov_pars_gg) {
			if (!gamma_gamma_model_) {
				Log::REFatal("Internal error: CalcGradientFGammaGamma called while Gamma-Gamma mode is disabled");
			}
			...
			if (eta_ptr == nullptr) {
				if (!has_covariates_) {
					Log::REFatal("Gamma-Gamma: no fixed effects / covariates available to build eta");
				}
				if ((int)beta_.size() != num_covariates_) {
					Log::REFatal("Gamma-Gamma: beta_ size (%d) != num_covariates_ (%d)",
						(int)beta_.size(), (int)num_covariates_);
			...
			if (y_ptr == nullptr) {
				Log::REFatal("Gamma-Gamma: response data not available");
			}
			if (num_clusters_ != 1) {
				Log::REFatal("Gamma-Gamma: currently supports only one cluster");
			}
			...
			vec_t Sg = vec_t::Zero(G);
			vec_t Ng = vec_t::Zero(G);
			...
			for (data_size_t i = 0; i < num_data_; ++i) {
				...
				const double n_alpha_plus_delta = Ng[g] * alpha + delta;
				const double delta_plus_S = delta + Sg[g];
				y[i] = -alpha + n_alpha_plus_delta * exp_eta * yi / delta_plus_S;
			}
		}//end CalcGradientFGammaGamma
		...
		*/
		void CalcGradientF(double* y,
			...
			//Leopold:
			if (gamma_gamma_model_) {
				CalcGradientFGammaGamma(y, fixed_effects, cov_pars);
				return;
			}
		...
		*/
		...
		//Leopold:
		// In this model, cov_pars[0] is delta and likelihood_additional_param_ is alpha.
		void PredictGammaGamma(const double* y_obs,
								...
								const vec_t& cov_pars_gg) {
			if (!gamma_gamma_model_) {
				Log::REFatal("Internal error: PredictGammaGamma called while Gamma-Gamma mode is disabled");
			}

			if (!predict_response) {
				Log::REFatal("Gamma-Gamma: latent prediction not supported");
			}
			if (re_group_data_pred == nullptr) {
				Log::REFatal("Gamma-Gamma: group_data_pred missing");
			}
			if (num_clusters_ != 1) {
				Log::REFatal("Gamma-Gamma: only one cluster supported");
			}
			if (num_re_group_total_ != 1 || num_gp_total_ != 0) {
				Log::REFatal("Gamma-Gamma: exactly one grouped RE and no GP required");
			}
			if (num_sets_re_ != 1) {
				Log::REFatal("Gamma-Gamma: only num_sets_re_ == 1 supported");
			}
			...
			const double* y_ptr = y_obs;
			if (y_ptr == nullptr) {
				if (!y_has_been_set_) {
					Log::REFatal("Gamma-Gamma: y not available");
				}
				y_ptr = y_vec_.data();
			}
			...
			if (eta_ptr == nullptr) {
				if (!has_covariates_) {
					Log::REFatal("Gamma-Gamma: no covariates available to build eta internally");
				}
				if ((int)beta_.size() != num_covariates_) {
					Log::REFatal("Gamma-Gamma: beta_ size mismatch");
				}
				eta_internal = X_ * beta_;
				eta_ptr = eta_internal.data();
			}
			...
			const int G = re_comp->GetNumUniqueREs();
			const auto& grp_obs = re_comp->random_effects_indices_of_data_;
			...
			#pragma omp parallel
			{
				...
				for (data_size_t i = 0; i < num_data_; ++i) {
					...
					const double exp_eta = std::exp(eta_ptr[i]);
					const int g = grp_obs[i];
			...
			vec_t mu_pred;
			if (has_covariates_) {
				if (covariate_data_pred == nullptr || coef_pred == nullptr) {
					Log::REFatal("Gamma-Gamma: covariate_data_pred or coef_pred missing");
				}
				const den_mat_t X_pred =
					Eigen::Map<const den_mat_t>(covariate_data_pred,
												num_data_pred,
												num_covariates_);
				const vec_t coef =
					Eigen::Map<const vec_t>(coef_pred, num_covariates_);
				mu_pred = X_pred * coef;
			}

			std::vector<std::vector<re_group_t>> re_group_levels_pred(
				1, std::vector<re_group_t>(num_data_pred));

			ConvertCharToStringGroupLevels(
				num_data_pred,
				1,
				re_group_data_pred,
				re_group_levels_pred);

			auto re_comp_group = std::dynamic_pointer_cast<RECompGroup<T_mat>>(re_comp);
			if (re_comp_group == nullptr) {
				Log::REFatal("Gamma-Gamma: random-effect component is not a grouped RE");
			}
			const auto& map_group_label_index = re_comp_group->map_group_label_index_;

			#pragma omp parallel for schedule(static)
			for (data_size_t i = 0; i < num_data_pred; ++i) {

				double eta_star = 0.0;

				if (fixed_effects_pred != nullptr) {
					eta_star += fixed_effects_pred[i];
				}
				if (has_covariates_) {
					eta_star += mu_pred[i];
				}

				const double exp_eta_star = std::exp(eta_star);
				const double inv_exp_eta_star = 1.0 / exp_eta_star;

				int g = -1;
				auto it = map_group_label_index->find(re_group_levels_pred[0][i]);
				if (it != map_group_label_index->end()) {
					g = it->second;
				}
				...
				double post_shape_theta;
				double post_rate_theta;
				if (g < 0 || g >= G) {
					post_shape_theta = delta;
					post_rate_theta = delta;
				}
				else {
					post_shape_theta = delta + Ng[g] * alpha;
					post_rate_theta = delta + Sg[g];
				}

				if (!(post_shape_theta > 1.0)) {
					mean = 1e100;
					if (predict_var) {
						var = 1e100;
					}
					...
					mean = alpha * inv_exp_eta_star * post_rate_theta / (post_shape_theta - 1.0);

					if (predict_var) {
						if (!(post_shape_theta > 2.0)) {
							var = 1e100;
						}
						else {
							const double second_moment =
								alpha * (alpha + 1.0) * inv_exp_eta_star * inv_exp_eta_star *
								post_rate_theta * post_rate_theta /
								((post_shape_theta - 1.0) * (post_shape_theta - 2.0));
							var = std::max(second_moment - mean * mean, 0.0);
						}
					}
				}
				...
				if (predict_var) {
					out_predict[i + num_data_pred] = var;
				}
			}
		}		
		...
		*/
		void Predict(const double* cov_pars_pred,
			...
			if (gamma_gamma_model_) {
				if (predict_cov_mat) {
					Log::REFatal("Predictive covariance matrix not supported for Gamma-Gamma model");
				}
				PredictGammaGamma(y_obs, num_data_pred, out_predict, predict_var, predict_response,
									re_group_data_pred, fixed_effects, fixed_effects_pred,
									covariate_data_pred, coef_pred, cov_pars);
				return;
			}
...
#pragma omp parallel for schedule(static) reduction(+:mean_lp_change, mean_lp_lag1, var_lp_change, cov_lp_lag1_lp_change)
	...
	private:
		...
		*/
		...
		//Leopold:
		bool gamma_gamma_model_ = false;
		double gamma_gamma_alpha_ = 1.;
		bool gamma_gamma_aux_pars_have_been_set_ = false;
		...
		*/
		void SetY(const float* y_data) {
				if (gauss_likelihood_) {
					...
					y_vec_ = vec_t(num_data_);
					for (int i = 0; i < num_data_; ++i) {
						y_vec_[i] = static_cast<double>(y_data[i]);
					}
					if (num_clusters_ == 1 && ((gp_approx_ != "vecchia" && gp_approx_ != "full_scale_vecchia") || vecchia_ordering_ == "none")) {
						y_[unique_clusters_[0]] = vec_t(num_data_);
						for (int i = 0; i < num_data_; ++i) {
						y_[unique_clusters_[0]][i] = static_cast<double>(y_data[i]);
					}
				}
				else {
					for (const auto& cluster_i : unique_clusters_) {
...
#pragma omp parallel for schedule(static)   
		...
		*/
		void InitializeDefaultSettings() {
			...
			if (!set_optim_config_has_been_called_ && NumAuxPars() > 0) {
				if (gamma_gamma_model_ || !gauss_likelihood_) {
		...
		*/
		void TransformAuxPars(const double* aux_pars_orig,
			...
			if (gamma_gamma_model_) {
				CHECK(aux_pars_orig != nullptr);
				CHECK(aux_pars_trans != nullptr);
				if (!(aux_pars_orig[0] > 0.)) {
					Log::REFatal("Gamma-Gamma: alpha must be > 0");
				}
		...
		*/
		void BackTransformAuxPars(const double* aux_pars_trans,
			...
			if (gamma_gamma_model_) {
				CHECK(aux_pars_trans != nullptr);
				CHECK(aux_pars_orig != nullptr);
				if (!std::isfinite(aux_pars_trans[0]) || !(aux_pars_trans[0] > 0.)) {
					aux_pars_orig[0] = 1e-12;
				}
				else {
		...
		double GetGammaGammaAlpha() const {
			double alpha = gamma_gamma_aux_pars_have_been_set_ ? gamma_gamma_alpha_ : likelihood_additional_param_;
		...
		double GetGammaGammaDelta(const vec_t& cov_pars_gg) const {
			CHECK((int)cov_pars_gg.size() >= 1);
			double delta = cov_pars_gg[0];
		...
		*/
		void UpdateLinCoef(vec_t& beta,
			...
			for (int ih = 0; ih < max_number_lr_shrinkage_steps_; ++ih) {
				...
				//Leopold:
				if (gamma_gamma_model_) {
					vec_t cov_pars_gg = vec_t::Zero(num_cov_par_);
					cov_pars_gg[0] = sigma2;
					EvalNegLogLikelihoodGammaGamma(nullptr, neg_log_likelihood_after_lin_coef_update_, fixed_effects_vec.data(), cov_pars_gg);
				}
				else if (gauss_likelihood_) {
