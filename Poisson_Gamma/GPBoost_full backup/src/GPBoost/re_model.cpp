...
namespace GPBoost {
	...
	void REModel::FindInitialValueBoosting() {
		...
		if (GetLikelihood() == std::string("poisson_gamma")) {
			return;
		}
		if (matrix_format_ == "sp_mat_t") {
	...
	void REModel::LineSearchLearningRate(const double* score,
		...
		CHECK(cov_pars_initialized_);
		if (GetLikelihood() == std::string("poisson_gamma")) {
			Log::REFatal("Line-search step lengths are not implemented for the Poisson-Gamma model");
		}
		if (matrix_format_ == "sp_mat_t") {
			re_model_sp_->OptimLinRegrCoefCovPar(nullptr,
				new_score,
	...
	*/
	...
	double REModel::TestNegLogLikelihoodPoissonGamma(const label_t* y_test,
		const double* fixed_effects_test,
		const data_size_t num_data) {
		if (matrix_format_ == "sp_mat_t") {
			return(re_model_sp_->TestNegLogLikelihoodPoissonGamma(y_test, fixed_effects_test, num_data, cov_pars_));
		}
		else if (matrix_format_ == "sp_mat_rm_t") {
			return(re_model_sp_rm_->TestNegLogLikelihoodPoissonGamma(y_test, fixed_effects_test, num_data, cov_pars_));
		}
		else {
			return(re_model_den_->TestNegLogLikelihoodPoissonGamma(y_test, fixed_effects_test, num_data, cov_pars_));
		}
	}
