...
namespace LightGBM {
	...
	ObjectiveFunction* ObjectiveFunction::CreateObjectiveFunction(const std::string& type, const Config& config) {
		...
		else if (type == std::string("poisson_gamma")) {
			return new RegressionL2loss(config);
		}
	...
	void ObjectiveFunction::InitGPModel(REModel* re_model,
		...
		if (!(re_model_->GaussLikelihood()) || re_model_->GetLikelihood() == std::string("poisson_gamma")) {
	...
	void ObjectiveFunction::LineSearchLearningRate(const double* score,
		...
		double& lr) const {
		if (has_gp_model_) {
			if (re_model_->GaussLikelihood() && re_model_->GetLikelihood() != std::string("poisson_gamma")) {//Gaussian likelihood
