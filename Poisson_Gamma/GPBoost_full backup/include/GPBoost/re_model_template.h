		...
		*/
		REModelTemplate(data_size_t num_data,
			...
			if (likelihood == nullptr) {
				poisson_gamma_model_ = false;
				likelihood_strg = "gaussian";
			}
			else {
				string_t likelihood_parsed = Likelihood<T_mat, T_chol>::ParseLikelihoodAlias(std::string(likelihood));
				poisson_gamma_model_ = (likelihood_parsed == "poisson_gamma");
				likelihood_strg = poisson_gamma_model_ ? "gaussian" : likelihood_parsed;
			}
...
//				}
		...
		*/
		void SetLikelihood(const string_t& likelihood) {
			...
			poisson_gamma_model_ = (likelihood_parsed == "poisson_gamma");
			string_t likelihood_strg = poisson_gamma_model_ ? "gaussian" : likelihood_parsed;
			...
			InitializeLikelihoods(likelihood_strg);
			if (!poisson_gamma_model_) {
				poisson_gamma_y_vec_.resize(0);
				poisson_gamma_y_has_been_set_ = false;
			}
			DetermineCovarianceParameterIndicesNumCovPars();
			InitializeDefaultSettings();
		...
		*/
		void OptimLinRegrCoefCovPar(const double* y_data,
			...
			// and for the custom Poisson-Gamma path, cov_pars[0] is the model parameter gamma, we set profile_out_error_variance_=False
			// we also set the optimizer of the regression coefficients to "gradient_descent", 
			// as within the Poisson-Gamma setting, we provides the gradient for beta and we want to avoid Gaussian closed-form/WLS coefficient updates
			if (poisson_gamma_model_) {
...
#pragma omp critical
			...
			MaybeCachePoissonGammaResponse(y_data);
			...
			if (gauss_likelihood_ && !find_learning_rate_for_GPBoost_algo) {
				...
				if (!(poisson_gamma_model_ && poisson_gamma_y_has_been_set_)) {
				...
				else if (poisson_gamma_model_ && poisson_gamma_y_has_been_set_) {
					y_vec_ = poisson_gamma_y_vec_;
				}
			}
...
#pragma omp parallel for schedule(static)
		...
		*/
		...
		//Leopold:
		// In this model, cov_pars[0] is gamma.
		void CalcGradParsPoissonGamma(const vec_t& cov_pars,
									...
									const double* fixed_effects) {
			if (!poisson_gamma_model_) {
				Log::REFatal("Internal error: CalcGradParsPoissonGamma called while Poisson-Gamma mode is disabled");
			}

			
			if (num_cov_par_ <= 0) {
				Log::REFatal("Poisson-Gamma: expected num_cov_par_ >= %d but got %d", 1, (int)num_cov_par_);
			}
			const double gamma = cov_pars[0];
			const double* eta_ptr = fixed_effects;
			vec_t eta_internal;
			if (!(gamma > 0.0)) {
				Log::REFatal("Poisson-Gamma: poisson_gamma_shape_ (gamma) must be > 0");
			}
			if (eta_ptr == nullptr) {
				if (!has_covariates_) {
					Log::REFatal("Poisson-Gamma: need covariates to build eta internally when fixed_effects is null");
				}
				if ((int)beta_.size() != num_covariates_) {
					Log::REFatal("Poisson-Gamma: beta_ size (%d) != num_covariates_ (%d)",
			...
			if (num_clusters_ != 1) {
				Log::REFatal("Poisson-Gamma: currently supports only one cluster");
			}
			if (num_re_group_total_ != 1 || num_gp_total_ != 0) {
				Log::REFatal("Poisson-Gamma: currently supports exactly one grouped RE and no GP");
			}
			...
			if (calc_cov_aux_par_grad) {
				if ((int)grad_cov_aux_par.size() != num_cov_par_) {
			...
			if (y_has_been_set_) {
				...
				Log::REFatal("Poisson-Gamma: response y must be set before CalcGradParsPoissonGamma");
			}

			vec_t Sg = vec_t::Zero(G);
			vec_t Tg = vec_t::Zero(G);
			
			//Here we just calculate the statistics Sg = sum of y and Tg = sum of exp(eta) for each group g
			// which we need for our gradient calculations.
			#pragma omp parallel
			{
				vec_t Sg_loc = vec_t::Zero(G);
				vec_t Tg_loc = vec_t::Zero(G);

				#pragma omp for schedule(static)
				for (data_size_t i = 0; i < num_data_; ++i) {
					const double yi = y_ptr[i];
					if (!(yi >= 0.0)) {
						Log::REFatal("Poisson-Gamma: response must be nonnegative");
					}
			...
			//Here we calculate the gradient of gamma here and then transform it to the gradient of log(gamma) since we optimize on the log-scale for gamma. 
			//The gradient of log(gamma) is then stored in grad_cov_aux_par at the position 0.
			if (calc_cov_aux_par_grad) {
				double grad_gamma = 0.0;

				#pragma omp parallel for schedule(static) reduction(+:grad_gamma)
				for (int g = 0; g < G; ++g) {
					const double S = Sg[g];
					double T = Tg[g];
					...
					// digamma is the derivative of log(gamma function)
					grad_gamma += -GPBoost::digamma(gamma + S)
								+ GPBoost::digamma(gamma)
								- std::log(gamma) - 1.0
								+ std::log(gamma + T)
								+ (gamma + S) / (gamma + T);
				}

				const double grad_log_gamma = grad_gamma * gamma;
				grad_cov_aux_par[0] = grad_log_gamma;
			}

			//Here we calculate the gradient w.r.t. the regression coefficient beta.
			if (calc_beta_grad) {
				if (num_sets_re_ != 1) {
					Log::REFatal("Poisson-Gamma: currently supports only num_sets_re_ == 1 for beta gradient");
				}
				...
				for (data_size_t i = 0; i < num_data_; ++i) {
					...
					const double ratio = (gamma + Sg[g]) / (gamma + T);
					grad_eta[i] = exp_eta * ratio - yi;
				}
...
#pragma omp parallel for schedule(static)
		...
		*/
		void CalcCovFactorOrModeAndNegLL(const vec_t& cov_pars_in,
			...
			//Leopold:
			if (poisson_gamma_model_) {
				EvalNegLogLikelihoodPoissonGamma(nullptr, neg_log_likelihood_, fixed_effects, cov_pars);
				return;
			}
...
#pragma omp parallel for schedule(static)
		...
		//Leopold:
		// Note that in this model, cov_pars[0] is gamma.
		void EvalNegLogLikelihoodPoissonGamma(
			const double* y_data,
			double& negll,
			const double* fixed_effects,
			const vec_t& cov_pars_pg) {
			if (!poisson_gamma_model_) {
				Log::REFatal("Internal error: EvalNegLogLikelihoodPoissonGamma called while Poisson-Gamma mode is disabled");
			}
			
			CHECK(num_sets_re_ == 1);
			CHECK((int)cov_pars_pg.size() >= 1);
			if (num_cov_par_ <= 0) {
				Log::REFatal("Poisson-Gamma: expected num_cov_par_ >= %d but got %d", 1, (int)num_cov_par_);
			}
			const double gamma = cov_pars_pg[0];
			if (!(gamma > 0.0)) {
				Log::REFatal("Poisson-Gamma: poisson_gamma_shape_ (gamma) must be > 0");
			}
			const double* eta_ptr = fixed_effects;
			...
			if (eta_ptr == nullptr) {
				if (has_covariates_) {
					...
					if (X_.rows() != num_data_ || X_.cols() != num_covariates_) {
						Log::REFatal("Poisson-Gamma: X_ has wrong dimension (%d,%d), expected (%d,%d)",
								(int)X_.rows(), (int)X_.cols(), (int)num_data_, (int)num_covariates_);
					}
					eta_internal = X_ * beta_;
					eta_ptr = eta_internal.data();
				} else {
					eta_internal = vec_t::Zero(num_data_);
					eta_ptr = eta_internal.data();
				}
			}
			...
			if (poisson_gamma_y_has_been_set_) {
				y_ptr = poisson_gamma_y_vec_.data();
			}
			else if (y_ptr == nullptr) {
				Log::REFatal("Poisson-Gamma: y_data is null and original response has not been cached");
			}
			...
			#pragma omp for schedule(static)
				...
				// constants included for debugging correctness
				obs_loc -= yi * eta; // I removed the constant term std::lgamma(yi + 1.0)  that is part of the true expression for the NLL
									// as we do not need it for optimization and it can cause numerical issues for large yi
				}
			...
			double grp_part = 0.0;
			#pragma omp parallel for schedule(static) reduction(+:grp_part)
			for (int g = 0; g < G; ++g) {
				const double S = Sg[g];
				double T = Tg[g];
				if (!(T > 0.0)) T = 1e-12;
				...
				// This corresponds to:
				// NLL - sum_i log(y_i!) = sum_i [ - y_i eta_i]
				//       + sum_g [ -log Γ(gamma+Sg) + log Γ(gamma) - gamma log gamma + (gamma+Sg) log(gamma+Tg) ]
				grp_part += -std::lgamma(gamma + S) + std::lgamma(gamma)
							- gamma * std::log(gamma)
							+ (gamma + S) * std::log(gamma + T);
			}

			negll = obs_part + grp_part;
		}

		double TestNegLogLikelihoodPoissonGamma(
			const label_t* y_test,
			const double* fixed_effects_test,
			data_size_t num_data_test,
			const vec_t& cov_pars_pg) {
			if (!poisson_gamma_model_) {
				Log::REFatal("Internal error: TestNegLogLikelihoodPoissonGamma called while Poisson-Gamma mode is disabled");
			}
			if (y_test == nullptr || fixed_effects_test == nullptr) {
				Log::REFatal("Poisson-Gamma: test labels and fixed effects are required for test NLL");
			}
			if ((data_size_t)re_group_levels_pred_.size() < 1 || (data_size_t)re_group_levels_pred_[0].size() != num_data_test) {
				Log::REFatal("Poisson-Gamma: prediction group data must be set before calculating test NLL");
			}
			CHECK(num_sets_re_ == 1);
			CHECK((int)cov_pars_pg.size() >= 1);
			if (num_cov_par_ <= 0) {
			...
			const double gamma = cov_pars_pg[0];
			if (!(gamma > 0.0)) {
				Log::REFatal("Poisson-Gamma: gamma must be > 0");
			}
			...
			vec_t Tg = vec_t::Zero(num_groups);
			double obs_part = 0.0;

#pragma omp parallel
			{
				vec_t Sg_loc = vec_t::Zero(num_groups);
				vec_t Tg_loc = vec_t::Zero(num_groups);
				double obs_loc = 0.0;
...
#pragma omp for schedule(static)
				for (data_size_t i = 0; i < num_data_test; ++i) {
					...
					Sg_loc[g] += yi;
					Tg_loc[g] += std::exp(eta);
					obs_loc -= yi * eta;
					obs_loc += std::lgamma(yi + 1.0);
				}
...
#pragma omp parallel for schedule(static)   
		...
		*/
		void EvalNegLogLikelihood(const double* y_data,
			...
			if (poisson_gamma_model_) { 
				CHECK(cov_pars != nullptr);
				const vec_t cov_pars_vec = Eigen::Map<const vec_t>(cov_pars, num_cov_par_);
				EvalNegLogLikelihoodPoissonGamma(y_data, negll, fixed_effects, cov_pars_vec);
				return;
			}
		...
		*/
		...
		//Leopold:
		// In this model, cov_pars[0] is gamma.
		void CalcGradientFPoissonGamma(double* y,
									const double* fixed_effects,
									const vec_t& cov_pars_pg) {
			if (!poisson_gamma_model_) {
				Log::REFatal("Internal error: CalcGradientFPoissonGamma called while Poisson-Gamma mode is disabled");
			}
			...
			MaybeCachePoissonGammaResponse(y);
			const double* y_ptr = nullptr;
			if (poisson_gamma_y_has_been_set_) {
				y_ptr = poisson_gamma_y_vec_.data();
			}
			...
			if (y_ptr == nullptr) {
				Log::REFatal("Poisson-Gamma: response data not available");
			}
			if (num_clusters_ != 1) {
				Log::REFatal("Poisson-Gamma: currently supports only one cluster");
			}
			...
			for (data_size_t i = 0; i < num_data_; ++i) {
				...
				double T = Tg[g];
				if (!(T > 0.0)) {
					T = 1e-12;  // numerical guard
				}

				const double ratio = (gamma + Sg[g]) / (gamma + T);

				y[i] = exp_eta * ratio - yi;
			}
		}//end CalcGradientFPoissonGamma
		...
		*/
		void CalcGradientF(double* y,
			...
			//Leopold:
			if (poisson_gamma_model_) {
				CalcGradientFPoissonGamma(y, fixed_effects, cov_pars);
				return;
			}
		...
		*/
		...
		//Leopold:
		// In this model, cov_pars[0] is gamma.
		void PredictPoissonGamma(const double* y_obs,
								...
								const vec_t& cov_pars_pg) {
			if (!poisson_gamma_model_) {
				Log::REFatal("Internal error: PredictPoissonGamma called while Poisson-Gamma mode is disabled");
			}

			if (!predict_response) {
				Log::REFatal("Poisson-Gamma: latent prediction not supported");
			}
			if (re_group_data_pred == nullptr) {
				Log::REFatal("Poisson-Gamma: group_data_pred missing");
			}
			if (num_clusters_ != 1) {
				Log::REFatal("Poisson-Gamma: only one cluster supported");
			}
			if (num_re_group_total_ != 1 || num_gp_total_ != 0) {
				Log::REFatal("Poisson-Gamma: exactly one grouped RE and no GP required");
			}
			if (num_sets_re_ != 1) {
				Log::REFatal("Poisson-Gamma: only num_sets_re_ == 1 supported");
			}
			...
			if (poisson_gamma_y_has_been_set_) {
				...
				if (!y_has_been_set_) {
					Log::REFatal("Poisson-Gamma: y not available");
				}
				y_ptr = y_vec_.data();
			}
			...
			if (eta_ptr == nullptr) {
				if (!has_covariates_) {
					Log::REFatal("Poisson-Gamma: no covariates available to build eta internally");
				}
				if ((int)beta_.size() != num_covariates_) {
					Log::REFatal("Poisson-Gamma: beta_ size mismatch");
				}
				eta_internal = X_ * beta_;
				eta_ptr = eta_internal.data();
			}
			...
			if (has_covariates_) {
				if (covariate_data_pred == nullptr || coef_pred == nullptr) {
					Log::REFatal("Poisson-Gamma: covariate_data_pred or coef_pred missing");
				}
			...
			if (re_comp_group == nullptr) {
				Log::REFatal("Poisson-Gamma: random-effect component is not a grouped RE");
			}
		...
		*/
		void Predict(const double* cov_pars_pred,
			...
			if (poisson_gamma_model_) {
				if (predict_cov_mat) {
					Log::REFatal("Predictive covariance matrix not supported for Poisson-Gamma model");
				}
				PredictPoissonGamma(y_obs, num_data_pred, out_predict, predict_var, predict_response,
...
#pragma omp parallel for schedule(static) reduction(+:mean_lp_change, mean_lp_lag1, var_lp_change, cov_lp_lag1_lp_change)
	...
	private:
		...
		vec_t y_vec_;
		/*! \brief Cached original response data for the custom Poisson-Gamma model */
		vec_t poisson_gamma_y_vec_;
		...
		/*! \brief Indicates whether the original Poisson-Gamma response has been cached */
		bool poisson_gamma_y_has_been_set_ = false;
		...
		*/
		...
		//Leopold:
		bool poisson_gamma_model_ = false;
		...
		*/
		...
		void MaybeCachePoissonGammaResponse(const double* y_data) {
			if (!poisson_gamma_model_ || poisson_gamma_y_has_been_set_ || y_data == nullptr) {
				return;
			}
			int labels_look_valid = 1;
#pragma omp parallel for schedule(static) reduction(&:labels_look_valid)
			for (data_size_t i = 0; i < num_data_; ++i) {
				if (!(y_data[i] >= 0.0)) {
					labels_look_valid = 0;
				}
			}
			if (labels_look_valid) {
				poisson_gamma_y_vec_ = Eigen::Map<const vec_t>(y_data, num_data_);
				poisson_gamma_y_has_been_set_ = true;
			}
		}

		void MaybeCachePoissonGammaResponse(const float* y_data) {
			if (!poisson_gamma_model_ || poisson_gamma_y_has_been_set_ || y_data == nullptr) {
				return;
			}
			int labels_look_valid = 1;
#pragma omp parallel for schedule(static) reduction(&:labels_look_valid)
			...
			if (labels_look_valid) {
				poisson_gamma_y_vec_ = vec_t(num_data_);
#pragma omp parallel for schedule(static)
				for (data_size_t i = 0; i < num_data_; ++i) {
					poisson_gamma_y_vec_[i] = static_cast<double>(y_data[i]);
				}
				poisson_gamma_y_has_been_set_ = true;
			}
		}
		...
		*/
		void SetY(const float* y_data) {
			...
			if (gauss_likelihood_) {
				if (poisson_gamma_model_) {
					vec_t y_double(num_data_);
#pragma omp parallel for schedule(static)
					for (data_size_t i = 0; i < num_data_; ++i) {
						y_double[i] = static_cast<double>(y_data[i]);
					}
					SetY(y_double.data());
					return;
				}
...
#pragma omp parallel for schedule(static)   
		...
		*/
		void MaybeKeepVarianceConstant(const vec_t& cov_pars,
			...
			//Leopold:
			if(poisson_gamma_model_){ return;}
		...
		*/
		void UpdateLinCoef(vec_t& beta,
			...
			for (int ih = 0; ih < max_number_lr_shrinkage_steps_; ++ih) {
				...
				//Leopold:
				if (poisson_gamma_model_) {
					vec_t cov_pars_pg = vec_t::Zero(num_cov_par_);
					cov_pars_pg[0] = sigma2;
					EvalNegLogLikelihoodPoissonGamma(nullptr, neg_log_likelihood_after_lin_coef_update_, fixed_effects_vec.data(), cov_pars_pg);
				}
