		...
		void EvalNegLogLikelihoodPoissonGamma(
			...
			if (eta_ptr == nullptr) {
				if (has_covariates_) {
					...
					Log::REFatal("Poisson-Gamma: no fixed effects available");
				}
			}
			...
			if (y_ptr == nullptr) {
				if (!y_has_been_set_) {
				Log::REFatal("Poisson-Gamma: y_data is null and y has not been set before");
				}
				y_ptr = y_vec_.data();
			}
...
#pragma omp parallel for schedule(static)   
		...
		*/
		...
		void CalcGradientFPoissonGamma(double* y,
			...
			if (eta_ptr == nullptr) {
				if (!has_covariates_) {
					Log::REFatal("Poisson-Gamma: no fixed effects / covariates available to build eta");
				}
				if ((int)beta_.size() != num_covariates_) {
					Log::REFatal("Poisson-Gamma: beta_ size (%d) != num_covariates_ (%d)",
						(int)beta_.size(), (int)num_covariates_);
